...
type Screen = "welcome" | "quiz" | "plan" | "login" | "dash";
...
function dietOK(f: FoodItem, diet: string): boolean {
...
function condOK(f: FoodItem, cond: string): boolean {
...
function pickMeal(slot: string, target: number, di: number, used: Set<string>, ctx: MealCtx): FoodItem {
  ...
  if (!list.length) return { n:"Seasonal fruit & nuts", c:target, slot:[slot], reg:["all"], t:[] };
...
function buildPlan(profile: Profile): Plan {
  ...
  const days: PlanDay[] = (WEEK as readonly string[]).map((dn, di) => {
    ...
    let f = total ? cal / total : 1;
    f = Math.max(0.78, Math.min(1.28, f));
  ...
  const condClause = cond !== "None" && cond !== "Other"
    ? ` It's tuned to go easy on your ${COND_SHORT[cond] || cond} — still, please review it with your doctor.`
  ...
  return {
    ...
    goal: profile.goal || "General fitness",
    diet: profile.diet || "Pure veg",
    condition: cond, regLabel,
    tips: makeTips(profile, cal),
    days,
  };
}
    ...
    <svg width={size} height={size} viewBox="0 0 48 48" fill="none">
      <defs>
        <linearGradient id={id} x1="0" y1="0" x2="1" y2="1">
          <stop offset="0" stopColor="#22C16B" />
          <stop offset="1" stopColor="#0E8A4D" />
        </linearGradient>
      </defs>
...
function CalRing({ pct = 1, size = 110, color = "#fff", track = "rgba(255,255,255,0.25)", big, small }: CalRingProps) {
  const r = (size - 14) / 2;
  const c = 2 * Math.PI * r;
...
const MEAL_UI: Record<string, { Icon: React.ElementType; col: string }> = {
  "Breakfast":      { Icon: Sunrise,  col: "#F59E0B" },
  "Mid-morning":    { Icon: Apple,    col: "#F43F5E" },
  "Lunch":          { Icon: Utensils, col: "#1DAA61" },
  "Evening snack":  { Icon: Cookie,   col: "#8B5CF6" },
  "Dinner":         { Icon: Moon,     col: "#6366F1" },
  "Bedtime":        { Icon: Moon,     col: "#64748B" },
};
...
function Chip({ label, val }: { label: string; val: string }) {
  return (
    <div className="rounded-2xl bg-white border border-gray-100 px-3 py-2.5 shadow-sm">
      ...
      <div className="font-semibold text-gray-800 text-sm mt-0.5">{val}</div>
    </div>
  );
}
...
function Login({ profile, onDone, onBack }: {
  ...
  function submit() {
    ...
    const ex = sget<UserRecord>(k);
    if (mode === "signup") {
      if (ex) { setErr("Account exists — switch to Log in"); setBusy(false); return; }
      sset<UserRecord>(k, { pw: hash(pw), name: profile?.name || id });
    ...
    <Shell>
      <Card className="p-6 md:p-8">
        ...
        <button disabled={busy} onClick={submit}
          className="w-full py-3.5 rounded-2xl text-white font-bold disabled:opacity-60 shadow-md"
          style={{ background: GREEN }}>
          {mode === "signup" ? "Start tracking" : "Log in"}
        </button>
        <div className="flex items-center justify-between mt-4 text-sm">
          ...
          <button onClick={() => { setMode(mode === "signup" ? "login" : "signup"); setErr(""); }}
            className="font-semibold" style={{ color: GREEN }}>
            {mode === "signup" ? "I have an account" : "New here? Sign up"}
          </button>
        </div>
      </Card>
    </Shell>
...
function Dash({ session, plan, tracking, onUpdate, onLogout }: {
  ...
  plan: Plan | null;
  tracking: Tracking;
  onUpdate: (t: Tracking) => void;
  onLogout: () => void;
}) {
  const today = WEEK[(new Date().getDay() + 6) % 7];
  const [sel, setSel] = useState<DayName>(today);
  const t = tracking;
  const dd: DayTracking = (t[sel] as DayTracking) || { meals: {}, water: 0 };
  const dp = plan?.days?.find(d => d.day === sel);
  const cal = plan?.dailyCalories || 0;

  const toggle = (i: number) =>
    onUpdate({ ...t, [sel]: { ...dd, meals: { ...dd.meals, [i]: !dd.meals[i] } } });
  const setWater = (n: number) =>
    onUpdate({ ...t, [sel]: { ...dd, water: n } });
  const logW = (w: number) =>
    onUpdate({ ...t, weights: { ...(t.weights || {}), [new Date().toISOString().slice(0, 10)]: w } });
  ...
  const streak = WEEK.filter(d => {
    const x = t[d] as DayTracking | undefined;
            ...
            <Card className="p-5 mb-4">
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-bold text-gray-800 flex items-center gap-2">
                  <Droplet size={18} style={{ color: GREEN }} /> Water
                </h3>
              ...
              <div className="flex gap-1.5">
                {[...Array(8)].map((_, i) => (
                  <button key={i} onClick={() => setWater(i + 1 === dd.water ? i : i + 1)}
                    ...
                    style={{ background: i < dd.water ? GREEN : "#E5E7EB" }} />
                ))}
              </div>
            </Card>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (s) {
      setSession(s);
      const tr = sget<Tracking>(`eatbc:track:${s.id}`) || {};
      setTracking(tr);
      const p = sget<Plan>(`eatbc:plan:${s.id}`);
      if (p) setPlan(p);
      setScreen("dash");
    }
  }, []);
    ...
    <Shell>
      <Card className="p-8 text-center">
        ...
        <button onClick={() => setScreen("quiz")}
          className="w-full py-3.5 rounded-2xl text-white font-semibold inline-flex items-center justify-center gap-2 hover:opacity-90 transition shadow-md"
          style={{ background: GREEN }}>
          Start my assessment <ChevronRight size={18} />
        </button>
        <p className="text-xs text-gray-400 mt-6 flex items-center justify-center gap-1.5">
          <Stethoscope size={13} /> Not medical advice — for any condition, consult your doctor.
        </p>
      </Card>
    </Shell>
  );
      ...
      <Card className="p-6 md:p-8">
        ...
        <div className="mb-6">
          <div className="flex justify-between text-xs text-gray-400 mb-2">
            ...
            <span>{Math.round(((step + 1) / Q.length) * 100)}%</span>
          </div>
        ...
        <div className={cur.sub ? "" : "mt-5"}>
          ...
          {cur.type === "height" && (
            <div className="flex gap-3">
              <div className="flex-1">
                ...
                <input type="number" value={profile.heightFt || ""}
                  onChange={e => setProfile(p => ({ ...p, heightFt: e.target.value }))}
                  placeholder="5"
                  className="w-full mt-1 px-4 py-3.5 rounded-2xl border-2 border-gray-200 outline-none focus:border-green-500 text-lg"
                  autoFocus />
              </div>
              <div className="flex-1">
                <label className="text-xs text-gray-400">Inches</label>
                <input type="number" value={profile.heightIn || ""}
          ...
          {cur.type === "text" && (
            <input type="text" value={(profile[cur.k] as string) || ""}
              onChange={e => setVal(e.target.value)}
              placeholder={cur.ph}
              className="w-full px-5 py-3.5 rounded-2xl border-2 border-gray-200 outline-none focus:border-green-500 text-lg"
              autoFocus />
          )}
          {cur.type === "number" && (
            <input type="number" value={(profile[cur.k] as string) || ""}
              onChange={e => setVal(e.target.value)}
              placeholder={cur.ph}
              className="w-full px-5 py-3.5 rounded-2xl border-2 border-gray-200 outline-none focus:border-green-500 text-lg"
              autoFocus />
      ...
      <div style={{ animation: "eFade .5s ease both" }}>
        ...
        {plan.condition !== "None" && (
          <div className="rounded-2xl px-4 py-3 mb-4 flex items-start gap-2.5 text-sm" style={{ background: "#FFF7ED", color: "#9A3412" }}>
            ...
            <span>This plan is adjusted for <b>{plan.condition}</b>. Please confirm it with your doctor before starting.</span>
          </div>
        )}
        ...
        <Card className="p-6 text-center">
          ...
          <button onClick={() => setScreen("login")}
            className="px-8 py-3.5 rounded-2xl text-white font-bold inline-flex items-center gap-2 shadow-md hover:opacity-90 transition"
            style={{ background: GREEN }}>
            Accept the challenge 💪 <ChevronRight size={20} />
          </button>
        </Card>
        <p className="text-center text-xs text-gray-400 mt-4">
          EatBC gives general guidance, not medical advice. Consult a doctor or registered dietician for any health condition.
        </p>
      </div>
    </Shell>
  );
  ...
  if (screen === "login") return (
    <Login profile={profile} onDone={(sess) => {
      setSession(sess);
      sset<Session>("eatbc:session", sess);
      if (plan) sset<Plan>(`eatbc:plan:${sess.id}`, plan);
      const tr = sget<Tracking>(`eatbc:track:${sess.id}`) || {};
      setTracking(tr);
      setScreen("dash");
  ...
  if (screen === "dash" && session) return (
    <Dash session={session} plan={plan}
      tracking={tracking}
